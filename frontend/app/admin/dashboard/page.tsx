...
export default function AdminDashboard() {
  ...
  const fetchDashboard = async () => {
    try {
      ...
      const dashboardStats =
        data?.stats || data?.data?.stats || data?.data || null;

      const recentLeads =
        data?.recentLeads || data?.data?.recentLeads || [];
      ...
      setPendingLeads(
        Array.isArray(recentLeads)
          ? recentLeads.filter((lead: any) => lead.status === 'pending')
          : []
      );
  ...
  const handleVerify = async (id: string, quality: string) => {
    try {
      ...
      setPendingLeads((prev) => prev.filter((lead) => lead._id !== id));
          ...
          {CAT_DATA.map((category) => (
            <div key={category.name}>
              <div className="flex justify-between text-sm mb-1.5">
                ...
                <span className="font-semibold">{category.pct}%</span>
              </div>
