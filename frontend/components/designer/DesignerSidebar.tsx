...
import {
  LayoutDashboard, Star, BarChart2, User, CreditCard, LogOut, Bell
} from 'lucide-react';
...
const NAV_ITEMS = [
  { label: 'Live Leads', icon: LayoutDashboard, href: '/designer/dashboard' },
  { label: 'My Purchases', icon: Star, href: '/designer/leads' },
  { label: 'Analytics', icon: BarChart2, href: '/designer/analytics' },
  { label: 'Profile', icon: User, href: '/designer/profile' },
  { label: 'Transactions', icon: CreditCard, href: '/designer/transactions' },
  { label: 'Notifications', icon: Bell, href: '/designer/notifications' },
];
...
export function DesignerSidebar({ activeTab }: DesignerSidebarProps) {
  ...
  const { unreadNotifications, resetNotificationCount } = useSocket();
  ...
  const handleNavClick = (href: string) => {
    if (href === '/designer/notifications') {
  ...
  return (
    <aside className="w-60 bg-bg-2 border-r border-white/[0.07] flex flex-col min-h-screen pt-16 sticky top-0">
      <div className="mx-4 mt-5 mb-2 card p-4">
        ...
        <div className="font-head text-3xl font-extrabold text-yellow-400">
          {user?.credits ?? 0}
        </div>
        <div className="text-xs text-gray-500 mb-3">
          ≈ {Math.floor((user?.credits ?? 0) / 5)} leads
        </div>
      ...
      <nav className="flex-1 px-3 py-2">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          const isNotification = item.href === '/designer/notifications';
